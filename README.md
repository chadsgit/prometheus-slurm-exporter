...
A ready-to-import dashboard for this fork's metrics is provided at
[grafana/slurm-dashboard.json](grafana/slurm-dashboard.json). It covers
cluster-wide CPU/GPU allocation, node states, per-node CPU/memory, the job
queue, scheduler cycle times and backfill stats, and per-partition,
per-account and per-user breakdowns (including fair-share).

To import: in Grafana, go to **Dashboards -> New -> Import**, paste the JSON
or upload the file, and select your Prometheus datasource when prompted.

The dashboard JSON is generated by [grafana/build_dashboard.py](grafana/build_dashboard.py). Re-run it after adding or renaming any `slurm_*` metric:

```bash
python3 grafana/build_dashboard.py
```

For reference, the original [community dashboard](https://grafana.com/dashboards/4323)
for `vpenso/prometheus-slurm-exporter` is shown below:

...
// NewQueueCollector creates a Prometheus collector for Slurm job queue state.
...
// QueueCollector implements prometheus.Collector for squeue metrics.
...
func (qc *QueueCollector) Collect(ch chan<- prometheus.Metric) {
	data, err := slurm.Run("squeue", []string{"-a", "-r", "-h", "-o", "%A,%T,%r", "--states=all"})
	if err != nil {
		log.Printf("warn: squeue failed: %v", err)
		return
	}
